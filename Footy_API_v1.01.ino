...
void setup() {
  ...
  tft.fillScreen(TFT_BLACK);  // sets out the loading messages in a rectangle while waiting for the time
  ...
  if ((weekDay == 1) || (weekDay == 2) || (weekDay == 3)) {  // decides whether to use live tips or saved tips based on the days of the week. 0=Sunday
    tipsLive = 1;                                            // get tips from API
    Serial.println("Getting tips from API and writing to LittleFS");
  } else {
    tipsLive = 0;  // get tips from LittleFS
  ...
  // get_tip[] repair if tips don't match API call. Should be rare
