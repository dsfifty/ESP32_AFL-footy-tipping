...
uint16_t titleAltColor = TFT_PINK;      // colour of the title when not current round or year
...
// Layout Options
int titleX = 90;       // These are the parameters for the setting out of the display. Title x position
...
void setup() {
  ...
  int k = 0;
  while ((WiFi.status() != WL_CONNECTED) && (k < 100)) {  // Wait for connection
    delay(50);
    Serial.print(".");
    k++;
  }
...
void tipsRequest() {  // function to get the current tips
  ...
  for (JsonObject tip : doc[F("tips")].as<JsonArray>()) {
    ...
    i++;
    total = total + 1;
    if ((tipsLive == 0) && (rnd == currentRnd) && (year == currentYear) && (savedSource == tipSource)) {
...
void menuScreen() {
  ...
  uint16_t dataAltColor = TFT_PINK;
