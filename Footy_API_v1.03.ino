...
uint16_t timeColor = TFT_LIGHTGREY;     // colour of the game time.
  ...
  for (int i = 0; i < total; i++) {
    if (get_hteam[orderG[i]]) {
      ...
      if (get_complete[orderG[i]] != 0) {
        ...
        tft.setTextColor(timeColor, background);
        tft.print(gameTime[orderG[i]]);
        tft.setTextColor(teamColor, background);
      }
    }
  }
...
void loop() {
  ...
  setDelay();  // actions the delay based on game live or not
}
