...
int delHTTP = 500;                 // delay when using the GET command for tips, games and ladder if the API is slow (500ms is best)
    ...
    if (get_hteam[orderG[i]]) {
      ...
      if (get_complete[orderG[i]] != 0) {
        ...
        if ((get_complete[orderG[i]] > 25) && (get_complete[orderG[i]] <= 50)) {  // displays 2 dots if it is the second quarter
          tft.setCursor((scoreLoc + 73), ((i * lineSpacing) + line1Start - 8), 1);
          tft.print(".");
          tft.setCursor((scoreLoc + 73), ((i * lineSpacing) + line1Start - 14), 1);
          tft.print(".");
        }
        if ((get_complete[orderG[i]] > 50) && (get_complete[orderG[i]] <= 75)) {  // displays 3 dots if it is the third quarter
          ...
          tft.setCursor((scoreLoc + 73), ((i * lineSpacing) + line1Start - 14), 1);
          tft.print(".");
          tft.setCursor((scoreLoc + 67), ((i * lineSpacing) + line1Start - 8), 1);
          tft.print(".");
        }
        if ((get_complete[orderG[i]] > 75) && (get_complete[orderG[i]] < 100)) {  // displays 4 dots if it is the fourth quarter
          ...
          tft.setCursor((scoreLoc + 67), ((i * lineSpacing) + line1Start - 8), 1);
          tft.print(".");
          tft.setCursor((scoreLoc + 67), ((i * lineSpacing) + line1Start - 14), 1);
          tft.print(".");
        }
        tft.setTextColor(teamColor, background);
...
void loop() {
  ...
  if ((tipSet == 0) && ((weekDay == 1) || (weekDay == 2) || ((weekDay == 3) && hour < 20))) {  // decides whether to use live tips or saved tips
    tipsLive = 1;                                                                              // based on the days of the week and hour. 0=Sunday. 24 hour clock
  } else {                                                                                     // 1 = get tips from API
    tipsLive = 0;                                                                              // 0 = get tips from LittleFS
  }                                                                                            // inserting it here allows the changeover without a device reset
